...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} Foi de beize KKKKKKKK👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["555183181546@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* DarckZera\n*WPP:* wa.me/+555183181546\n*INSTA:* https://www.instagram.com/p/COYojGRhVno/?igshid=dh51crqedb0w'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, Que se foda todo mundo'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️Meu dono é um vagabundo e ainda não criou'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'hilih':
					...
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Cadê o Link vei?')
				...
				case 'ytsearch':
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Onde está o Link vei?')
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Onde tá o nome do usuário?', text, {quoted: mek})
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('Cadê o Link vei?')
				...
				case 'limpar':
					...
					reply('ATRASO DO KARAI🤬🤙 Clear Chat')
					break
				case 'ts':
